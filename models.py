...
class S2V_QN_1(torch.nn.Module):
    def __init__(self,reg_hidden, embed_dim, len_pre_pooling, len_post_pooling, T):
        ...
        self.mu_1 = torch.nn.Parameter(torch.Tensor(1, embed_dim))#only change 1 to 3 for 3 features
    ...
    def forward(self, xv, adj):
        ...
        for t in range(self.T):
            if t == 0:
                ...
                mu_1 = torch.matmul(xv, self.mu_1).clamp(0) #mu_1 = (1,17,64)
                #mu_1.transpose_(1,2)
                # before pooling:
                for i in range(self.len_pre_pooling):
                ...
                mu = torch.add(mu_1, mu_2).clamp(0) #(1,17,64)

#         q_1 = self.q_1(torch.matmul(xv.transpose(1,2),mu)).expand(minibatch_size,nbr_node,self.embed_dim)

        q_1 = self.q_1(torch.matmul(xv[0,:,0].reshape((1,nbr_node,1)).transpose(1,2),mu)).expand(minibatch_size,nbr_node,self.embed_dim)
        q_2 = self.q_2(mu)#q_2 = (1,17,64)
